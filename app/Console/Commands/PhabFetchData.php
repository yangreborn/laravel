...
class PhabFetchData extends Command
{
    ...
    protected $multiple_array = [5,5,5,5,5];
    ...
    protected $project_infos = [
        ...
        "NVR"=>[
            ...
            "phab_id"=>"172.16.0.124",
            "need_path"=>[          
                "trunk/nvrv7/nvr_vob/31-cbb_vstation",
                "trunk/nvrv7/nvr_vob/41-service_nvr",
                "trunk/nvrv7/nvr_vob/41-service_vstation",
                "trunk/nvrv7/nvr_vob/51-appext2",
                "trunk/nvrv7/nvr_vob/61-nms_nvr",
                "trunk/nvrv7/nvr_vob/61-nmsmedt",
            ]
        ],
        "NVR_JIXIAN"=>[
            "department"=>"JIXIAN",
            ...
            "phab_id"=>"172.16.0.124",
            "need_path"=>[          
                "trunk/nvrv7/nvr_vob/10-common",
                "trunk/nvrv7/nvr_vob/20-hal",
                "trunk/nvrv7/nvr_vob/30-cbb",
                "trunk/nvrv7/nvr_vob/50-app",
                "trunk/nvrv7/nvr_vob/60-nms",
            ]
        ],
        "NVR_WUXIAN"=>[
            ...
            "phab_id"=>"172.16.0.124",
            "need_path"=>[          
                "trunk/nvrv7/nvr_vob/41-service_wd",
                "trunk/nvrv7/nvr_vob/61-nms_wd",
            ]
        ],
    ];
    ...
    public function get_repo_data(){
        ...
        for($x=0;$x<5;$x++){
            $tmp_repos = array();
            $repo_datas[$x] = $this->pdo_mysql($this->mysql_con[$x][0],$select);#连接数据库
...
#########################评审表数据获取1##############################################
    ...
    public function get_audit_data($mysql_con){
        $update_time = $this->get_update_time($this->mpl_dbname[2]);#获取上次插入最新数据时间
        $audit_datas = array();
        $select = "select id,phid,authorPHID,dateCreated,transactionType,newValue,commentPHID,objectPHID from audit_transaction where dateModified >=".$update_time;
